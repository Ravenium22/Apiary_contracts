...
interface IERC20Mintable {
    ...
    function mintToTreasury(address acount, uint256 amount_) external;
...
contract BeraReserveTreasury is Ownable {
    ...
    event DebtUpdated(uint256 indexed totalDebt);
    ...
    address public sBRR;
    uint256 public sBRRQueue; // Delays change to sBRR address
    ...
    constructor(address _BRR, address _USDC, uint256 _blocksNeededForQueue) {
        ...
        ///@notice intially unbacked by treasury
        totalDebt = 150_000e9;
    }

    /**
     * @notice allow approved address to deposit an asset for BRR
     *     @param _amount uint
    ...
    function deposit(uint256 _amount, address _token, uint256 _profit) external returns (uint256 send_) {
        ...
        // mint BRR needed and store amount of rewards for distribution
    ...
    /**
     * @notice allow approved address to burn BRR for reserves
     *     @param _amount uint
     *     @param _token address
     */
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sBRR).balanceOf(msg.sender); // Can only borrow against sBRR held
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with BRR
     *     @param _amount uint
     */
    function repayDebtWithBRR(uint256 _amount) external {
        ...
        emit RepayDebt(msg.sender, BRR, _amount, _amount);
    }
    ...
    // // //!needed??
    // function updateDebt(uint256 _debt) external {
    //     require(isDebtor[msg.sender], "Not approved");
    //     totalDebt = totalDebt.add(_debt);
    //     emit DebtUpdated(totalDebt);
    // }

    /**
     * @notice send epoch reward to staking contract
     *     @param _recipient Address receiving the minted BRR tokens
     *     @param _amount Amount of BRR tokens to mint
     */
    ...
    /// @notice Returns the amount of treasury reserves not needed to back circulating BRR tokens
    /// @dev Excess = totalReserves - (totalSupply - totalDebt)
    /// @return Amount of excess reserves available for minting rewards (rebase for stakers)
    /// @dev basically We’ll only print BRR tokens if we actually have more reserves than we need
    /// @dev the surplus will be shared with stakers as rebase
    ...
    /**
     * @notice returns BRR valuation of asset
     *     @param _token address
     *     @param _amount uint
     *     @return value_ uint
     */
    ...
    function queue(MANAGING _managing, address _address) external onlyManager returns (bool) {
        ...
        } else if (_managing == MANAGING.SBRR) {
            // 9
            sBRRQueue = block.number.add(blocksNeededForQueue);
        } else {
            return false;
        }
            ...
            if (requirements(reserveTokenQueue, isReserveToken, _address)) {
                ...
                if (!listContains(reserveTokens, _address)) {
                    if (!listContains(liquidityTokens, _address)) {
                        ...
                        revert("TOKEN IS LIQUIDITY TOKEN");
                    }
                }
            }
