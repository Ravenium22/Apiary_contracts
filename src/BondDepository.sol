...
interface IStakingHelper {
    ...
    function stakeFor(uint256 _amount, address staker) external;
}

import { Pausable } from "@openzeppelin/contracts/utils/Pausable.sol";

contract BeraReserveBondDepository is Ownable, Pausable {
    ...
    address public immutable BRR; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints BRR when receives principle
    ...
    constructor(address _brr, address _principle, address _treasury, address _DAO, address _bondCalculator) {
        require(_brr != address(0), "BondDepository: _brr cannot be address(0)");
        BRR = _brr;
        require(_principle != address(0), "BondDepository: _principle cannot be address(0)");
        principle = _principle;
        require(_treasury != address(0), "BondDepository: _treasury cannot be address(0)");
        treasury = _treasury;
        require(_DAO != address(0), "BondDepository: _DAO cannot be address(0)");
        DAO = _DAO;
     ...
     *  @param _buffer uint
     */
    function setAdjustment(bool _addition, uint256 _increment, uint256 _target, uint256 _buffer) external onlyPolicy {
        require(_increment <= terms.controlVariable.mul(25).div(1000), "Increment too large");

        adjustment =
            Adjust({ add: _addition, rate: _increment, target: _target, buffer: _buffer, lastBlock: block.number });
    }
     ...
     *  @return uint
     */
    function deposit(uint256 _amount, uint256 _maxPrice, address _depositor) external whenNotPaused returns (uint256) {
        require(_depositor != address(0), "Invalid address");

        decayDebt();
        require(totalDebt <= terms.maxDebt, "Bond max capacity reached");
        ...
        require(payout >= 10_000_000, "Bond too small"); // must be > 0.01 BRR ( underflow protection )
        require(payout <= maxPayout(), "Bond too large"); // size protection because there is no slippage

        // profits are calculated
        uint256 fee = payout.mul(terms.fee).div(10_000);
         ...
         *         approved and
         *         deposited into the treasury, returning (_amount - profit) BRR
         */
        ...
        bondInfo[_depositor] = Bond({
            amountBonded: bondInfo[_depositor].amountBonded.add(_amount),
     ...
     *  @return uint
     */
    function redeem(address recipient, bool _stake) external whenNotPaused returns (uint256) {
        Bond memory info = bondInfo[msg.sender];
        uint256 percentVested = percentVestedFor(msg.sender); // (blocks since last interaction / vesting term remaining)

        if (percentVested >= 10000) {
            // if fully vested
            delete bondInfo[msg.sender]; // delete user info
            emit BondRedeemed(recipient, info.payout, 0); // emit bond data
            return stakeOrSend(recipient, _stake, info.payout); // pay user everything due
        } else {
            // if unfinished
            ...
            bondInfo[msg.sender].payout = info.payout.sub(payout);
            bondInfo[msg.sender].vesting = info.vesting.sub(block.number.sub(info.lastBlock));
            bondInfo[msg.sender].lastBlock = block.number;
            bondInfo[msg.sender].pricePaid = info.pricePaid;

            emit BondRedeemed(recipient, payout, bondInfo[msg.sender].payout);
            return stakeOrSend(recipient, _stake, payout);
        }
    }
    ...
    function stakeOrSend(address _recipient, bool _stake, uint256 _amount) internal returns (uint256) {
        if (!_stake) {
            ...
            IERC20(BRR).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(BRR).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stakeFor(_amount, _recipient);
            } else {
                IERC20(BRR).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(BRR).totalSupply().mul(terms.maxPayout).div(100_000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to BRR supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(BRR).totalSupply();
    ...
    function getBondInfo(address _depositor)
        external
        view
        returns (uint256 amountBonded, uint256 payout, uint256 vesting, uint256 lastBlock, uint256 pricePaid)
    {
        Bond memory bond = bondInfo[_depositor];

        return (bond.amountBonded, bond.payout, bond.vesting, bond.lastBlock, bond.pricePaid);
    }

    /* ======= AUXILLIARY ======= */

    /**
     *  @notice allow anyone to send lost tokens (excluding principle or BRR) to the DAO
     *  @return bool
     */
