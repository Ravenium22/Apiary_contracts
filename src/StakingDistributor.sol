...
contract Distributor is Policy {
    ...
    constructor(address _treasury, address _BRR, uint256 _epochLength, uint256 _nextEpochBlock) {
        ...
        if (nextEpochBlock <= block.number) {
            ...
            for (uint256 i = 0; i < info.length; i++) {
                if (info[i].rate > 0) {
                    ...
                    info[i].recipient, nextRewardAt(info[i].rate));
    ...
    function circulatingTotalSupply() public view returns (uint256) {
        uint256 totalSupply = IERC20(BRR).totalSupply();
        return totalSupply.sub(150_000e9);
    }
