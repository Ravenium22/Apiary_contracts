...
interface IsBRR {
    function rebase(uint256 BRRProfit_, uint256 epoch_) external returns (uint256);
...
interface IBR is IERC20 {
    ...
    function setTreasuryValue(uint256 _treasuryValue) external;

    function setTreasury(address _treasury) external;

    function setFeeDistributor(address _feeDistributor) external;

    function setDecayRatio(uint256 _decayRatio) external;

    function excludeAccountsFromFees(address account, bool excluded) external;

    function excludeAccountsFromDecay(address account, bool excluded) external;

    function excludeMultipleAccountsFromFees(address[] calldata accounts, bool excluded) external;

    function setBuySellFee(uint256 buyFee, uint256 sellFee) external;

    function setFeeDisabled(bool _isFeeDisabled) external;

    function marketCap() external view returns (uint256 marketCapValue);
...
contract BeraReserveStaking is Ownable, Pausable {
    using SafeMath for uint256;
    ...
    /**
     * @notice stake BRR to enter warmup
     *     @param _amount uint
     *     @return bool
     */
    ...
    /**
     * @notice retrieve sBRR from warmup
    ...
    function claim(address _recipient) public whenNotPaused {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            IWarmup(warmupContract).retrieve(_recipient, IsBRR(sBRR).balanceForGons(info.gons));
        }
    }
    ...
    /**
     * @notice forfeit sBRR in warmup and retrieve BRR
     */
    ...
    /**
     * @notice returns the sBRR index, which tracks rebase growth
     *     @return uint
     */
    ...
    /**
     * @notice returns contract BRR holdings, including bonuses provided
     *     @return uint
     */
