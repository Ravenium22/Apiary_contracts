...
contract MockBeraReserveTreasury is Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        // isLiquidityToken[ _BRRDAI ] = true;
        // liquidityTokens.push( _BRRDAI );
    ...
    function deposit(uint256 _amount, address _token, uint256 _profit) external returns (uint256 send_) {
        ...
        IERC20Mintable(BRR).mint(msg.sender, send_);
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(BRR).mint(_recipient, _amount);
    ...
    function auditReserves() external onlyManager {
        ...
        for (uint256 i = 0; i < liquidityTokens.length; i++) {
            reserves =
                reserves.add(valueOfToken(liquidityTokens[i], IERC20(liquidityTokens[i]).balanceOf(address(this))));
        }
