...
contract USDC is LibNote {
    ...
    uint256 public USDClyUSDCLimit;
    ...
    function mint(address usr, uint256 wad) external {
        if (wards[msg.sender] == 0) {
            require(
                add(wad, USDCMintedToday[msg.sender]) <= USDClyUSDCLimit
                    || (sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= USDClyUSDCLimit),
                "Over USDCly USDC Limit"
            );
